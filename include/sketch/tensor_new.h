...
namespace SeqSketch {
    ...
    void tensor2_sketch(const Seq<seq_type> &seq, Vec<embed_type> &embedding, const Tensor2Params &params) {
    ...
    template<class seq_type, class embed_type>
    void tensor2_slide_sketch(const Vec<seq_type> &seq, Vec2D<embed_type> &embedding, const Tensor2_slide_Params &params) {
        ...
        for (int i = 0; i < seq.size(); i++) {
            ...
            if ((i + 1) % params.stride == 0) {
                ...
                for (int m = 0; m < params.embed_dim; m++) {
                    embed_type prod = 0;
                    for (int r = 0; r < params.num_phases; r++) {
                        prod += ((r % 2 == 0) ? 1 : -1) * M[1][params.tup_len][m * params.num_phases + r];
                    }
                    em[m] = prod;
                }
    ...
    template<class seq_type, class embed_type>
    void conv_slide_sketch(const Vec2D<seq_type> &seq, Vec2D<embed_type> &embedding, const Tensor2_slide_Params &params) {
