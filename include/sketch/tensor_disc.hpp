...
namespace SeqSketch {
    ...
    void tensor_disc_sketch(const Seq<seq_type> &seq, Vec2D<embed_type> &embedding, const TensorParams &params) {
    ...
    Vec3D<embed_type> tensor_disc_slide(const Seq<seq_type> &seq, const TensorSlideParams &params) {
        ...
        for (int m = 0; m < params.embed_dim; m++) {
            Vec3D<embed_type> cnt = new3D<embed_type>(params.tup_len, params.tup_len, params.num_phases, 0);
            for (int i = 0; i < seq.size(); i++) {
                ...
                if (j >= 0) {
                    for (int t = 0; t < params.tup_len; t++) {
                        ...
                        for (int t2 = t - 1; t2 >= 0; t2--) {
                            ...
                            //                                    cnt[t2][t][shift] -= cnt[t2 + 1][t][p] * seq[j][p2];
    ...
    template<class seq_type, class embed_type>
    Vec3D<embed_type> tensor_disc_slide3(const Vec3D<seq_type> &seq, const Vec<TensorSlideParams> &params) {
