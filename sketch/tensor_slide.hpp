...
void tensor_slide_sketch(const Seq<seq_type> &seq,
                         Vec2D<embed_type> &embedding,
                         const TensorSlideParams &params) {
    ...
    for (int m = 0; m < params.embed_dim; m++) {
        ...
        for (size_t i = 0; i < seq.size(); i++) {
            ...
            if (sketch_now(i, seq.size(), params.stride, params.offset)) {
                ...
                embed_type bin = std::upper_bound(params.bins.begin(),
                                                  params.bins.begin() + params.num_bins, prod)
                        - params.bins.begin();
