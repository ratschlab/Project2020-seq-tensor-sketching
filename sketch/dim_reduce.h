...
struct flatten_int32 {
    flatten_int32() {}

    flatten_int32(uint32_t flat_dim, uint32_t sketch_dim, uint32_t max_len) :
    ...
    static double dist(const std::vector<uint32_t> &v1, const std::vector<uint32_t> &v2) {
        Timer timer("flatten_int32_dist");
...
struct flatten_double {
    flatten_double() {}

    flatten_double(uint32_t output_dim, uint32_t input_dim, uint32_t input_max_len) :
          flat_dim(output_dim),sketch_dim(input_dim), max_len(input_max_len) {
        assert((this->flat_dim) % this->sketch_dim == 0 && "input_dim must be divisible by inner_dim");
        std::random_device rd;
        std::mt19937 gen(rd());
    ...
    static double dist(const std::vector<double> &v1, const std::vector<double> &v2) {
        Timer timer("flatten_double_dist");
        assert(v1.size()==v2.size());
        std::vector<double> d(v1.size());
        for (size_t i = 0; i < d.size(); i++) {
            d[i] = abs(v1[i] - v2[i]);
