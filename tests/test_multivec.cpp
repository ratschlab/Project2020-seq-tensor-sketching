...
namespace {
template <typename T> class Pow : public ::testing::Test {};
...
TYPED_TEST(Pow, Zero) {
  ...
  for (uint32_t i = 0; i < 10; ++i) {
    EXPECT_EQ(1, SeqSketch::int_pow<TypeParam>(dist(rng), 0));
  }
}
...
TYPED_TEST(Pow, Random) {
  ...
  for (uint32_t i = 0; i < 10; ++i) {
    ...
    EXPECT_EQ(std::pow(base, exp), SeqSketch::int_pow<TypeParam>(base, exp));
  }
}
