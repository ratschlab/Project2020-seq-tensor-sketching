...
struct SketchModule : public BasicModule {
    ...
    void override_model_params() override {
        tensor_slide_params.embed_dim = embed_dim;
    }
        ...
        for (int si = 0; si < num_seqs; si++) {
            ...
            if (sketch_method == "TenSlide") {
                ...
                for (int i = offset; i < sketch_end(offset, kmers.size()); i += stride) {
                    ...
                    Vec<seq_type> kmer_slice(kmers.begin() + i, std::min(kmers.begin() + i + win_len, kmers.end()));
