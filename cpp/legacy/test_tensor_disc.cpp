...
struct TestModule1 {
    ...
    void compute_sketches() {
        ...
        for (int si = 0; si < num_seqs; si++) {
            ...
            //            tensor_disc_sketch<int, double>(seqs[si], ten_disc_sketch[si], discModules.tensor_params);
            //            Vec3D<double> in, out;
            slide_disc_sketch1[si] = tensor_disc_slide<int, double>(seqs[si], lay0);
            slide_disc_sketch2[si] = tensor_disc_slide3<double, double>(slide_disc_sketch1[si], lay1);
            slide_disc_sketch3[si] = tensor_disc_slide3<double, double>(slide_disc_sketch2[si], lay2);
    ...
    void save_output() {
        ...
        for (int i = 0; i < seqs.size(); i++) {
            for (int j = i + 1; j < seqs.size(); j++) {
                fo << dists[0][i][j] << ", " << dists[1][i][j] << ", " << dists[2][i][j] << ", " << dists[3][i][j] << ", " << dists[4][i][j] << ", " << dists[5][i][j] << ", " << dists[6][i][j] << "\n";
            }
        }
        fo.close();
    }
