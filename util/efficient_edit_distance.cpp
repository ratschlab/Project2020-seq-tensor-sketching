...
//    // init conv_dist = (l2+1)x(l1+1)
//    std::vector<std::vector<unsigned int>> conv_dist(l2+1, std::vector<unsigned int>(l1+1, (l1+l2)*2));
//    for(size_t i=0;i <= l1;i++) {
//        conv_dist[0][i] = i;
//    }
//    for(size_t j=0;j <= l2;j++) {
...
//            size_t t = MIN((dist[i-1][j]+1),(conv_dist[i][j-1]+1));
//            dist[i][j] = MIN(t,(conv_dist[i-1][j-1]+track));
//        }
//    }
//    return conv_dist[l2][l1];
//}
//
...
//        d0[i] = i;
//    }
//    for(size_t i=1;i<=l2;i++) { // d1 = row conv_dist[i,:], d0 = row  [i-1,:]
//        d0[0] = i-1;
//        d1[0] = i;
//        for (size_t j=1;j<=l1;j++) {
//            size_t track, t;
