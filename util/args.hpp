...
struct Parser {
    ...
    std::map<std::string, Argument> name2arg;

    bool not_exists(const std::string &name) {
...
static const Argument MAX_NUM_BLOCKS = { L_MAX_NUM_BLOCKS, S_MAX_NUM_BLOCKS, ArgType::INT,
                                         "maximum number of blocks for block permutation" };
static const Argument MIN_NUM_BLOCKS = { L_MIN_NUM_BLOCKS, S_MIN_NUM_BLOCKS, ArgType::INT,
                                         "minimum number of blocks for block permutation" };
static const Argument SEQUENCE_SEEDS = { L_SEQUENCE_SEEDS, S_SEQUENCE_SEEDS, ArgType::INT,
                                         "number of initial random sequences " };
static const Argument MUTATION_PATTERN = { L_MUTATION_PATTERN, S_MUTATION_PATTERN, ArgType::STRING,
                                           "the mutational pattern, can be 'linear', or 'tree'" };
static const Argument BLOCK_MUTATION_RATE
        = { L_BLOCK_MUTATION_RATE, S_BLOCK_MUTATION_RATE, ArgType::FLOAT,
            "the probability of having a block permutation" };
static const Argument MUTATION_RATE = { L_MUTATION_RATE, S_MUTATION_RATE, ArgType::FLOAT,
                                        "rate of point mutation rate for sequence generation" };
static const Argument NUM_PHASES
        = { L_NUM_PHASES, S_NUM_PHASES, ArgType::INT,
            "number of phase to be used for modular arithmetic in tensor sketching" };
static const Argument NUM_BINS = { L_NUM_BINS, S_NUM_BINS, ArgType::INT,
                                   "number of bins for descritization after tensor sketching" };
static const Argument KMER_SIZE = { L_KMER_SIZE, S_KMER_SIZE, ArgType::INT,
                                    "kmer length, used for sequence to kmer transformation" };
static const Argument TUPLE_ON_KMER
        = { L_TUPLE_ON_KMER, S_TUPLE_ON_KMER, ArgType::INDIC,
            "apply tuple-based methods (OMH, TensorSketch, and TenSlide), on kmer sequence" };
static const Argument TUP_LEN
        = { L_TUP_LEN, S_TUP_LEN, ArgType::INT,
            "ordered tuple length, used in ordered MinHash and Tensor-based sketches" };
static const Argument EMBED_DIM = { L_EMBED_DIM, S_EMBED_DIM, ArgType::INT,
                                    "embedding dimension, used for all sketching methods" };
static const Argument WIN_LEN
        = { L_WIN_LEN, S_WIN_LEN, ArgType::INT,
            "window length: the size of sliding window, which will be all sketched" };
static const Argument STRIDE = { L_STRIDE, S_STRIDE, ArgType::INT,
                                 "stride for sliding window: shift step for sliding window" };
static const Argument OFFSET
        = { L_OFFSET, S_OFFSET, ArgType::INT, "initial index to start the sliding window" };
static const Argument SKETCH_METHOD
        = { L_SKETCH_METHOD, S_SKETCH_METHOD, ArgType::STRING,
            "the sketching method to use, options are MH, WMH, OMH, TenSketch, TenSlide" };
static const Argument DIRECTORY = { L_DIRECTORY, S_DIRECTORY, ArgType::STRING,
                                    "working directory for input/output reading/writing" };
...
static const Argument FORMAT_INPUT = { L_FORMAT_INPUT, S_FORMAT_INPUT, ArgType::STRING,
                                       "input format, options: 'fasta', 'csv'" };
...
struct VirtualArgSet : public Parser {
    ts::Vec<std::string> method_names_short
            = { "ED", "MH", "WMH", "OMH", "TenSketch", "TenSlide", "Ten2", "Ten2Slide" };
    Vec<std::string> method_names_long
            = { "edit distance", "MinHash",      "Weighted MinHash", "Ordered MinHash",
                "Tensor Sketch", "Slide Sketch", "Tensor Sketch v2", "Slide Sketch v2" };
    ...
    int sequence_seeds;
    std::string mutation_pattern;
    // sketching
    int kmer_size;
    std::string sketch_method;
    int embed_dim;
    int tup_len;
    bool tuple_on_kmer;
    int num_phases;
